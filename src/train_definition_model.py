...
import config
from arora import SIF
from evaluate import eval_rank
from data_utils import ReverseDictionary, Crossword, pad_sequence, token_ids_to_sentence
...
tf.app.flags.DEFINE_integer("vocab_size", 98202, "Nunber of words the model"
                                                 "knows and stores representations for")
tf.app.flags.DEFINE_integer("num_epochs", 20, "Train for this number of"
...
tf.app.flags.DEFINE_string("train_file", "train.definitions.ids",
                           "File with dictionary definitions for training.")
tf.app.flags.DEFINE_string("dev_file", "dev.definitions.ids",
                           "File with dictionary definitions for dev testing.")
tf.app.flags.DEFINE_string("save_dir", "../saved/rnn-tf/recurrent_9.ckpt", "Directory for saving model."
...
tf.app.flags.DEFINE_string("embeddings_path",
                           "../data/embeddings/glove_vocab.840B.300d.-1.pkl",
...
def read_data(data_path, vocab_size, phase="train"):
    ...
    gloss_path = os.path.join(
        data_path, "%s.definitions.ids.gloss" % (phase))
    head_path = os.path.join(
        data_path, "%s.definitions.ids.head" % (phase))
...
def evaluate_model(sess, data_dir, input_node, target_node, prediction,
                   loss, rev_vocab, embs, out_form="cosine"):
    ...
    #     sent_vec = sess.run(prediction, feed_dict={input_node: X})
    #
    #     word_vec = np.copy(embs)
    #     idx_rank, candidate_ids = eval_rank(sent_vec, word_vec, y)
    #
    #     concat_vec = sif.remove_pc(np.vstack((word_vec[2:], sent_vec)), test_params['num_pc'])
    ...
    for test_type, test_params in config.tests['crossword'].iteritems():
        ...
        print ("RNN on crossword")
        eval_rank(sent_vec, word_vec, y, idx_to_word=dataset.idx_to_word, lengths=dig)
        ...
        # eval
        # sims = 1 - np.squeeze(dist.cdist(y_embs, word_vec, metric="cosine"))
        ...
        # print (candidate_ids)
        # # print (y_embs.shape, word_vec.shape)
        # # eval_rank(y_embs, word_vec, y)


def restore_model(sess, save_dir, dataset, out_form):
    model_path = save_dir  # tf.train.latest_checkpoint(save_dir)
    ...
    vocab, rev_vocab = dataset.word_to_idx, dataset.idx_to_word
    return input_node, target_node, predictions, loss, vocab, rev_vocab


def query_model(sess, input_node, predictions, vocab, rev_vocab, dataset,
    ...
    while True:
        ...
        token_ids = dataset.sentence_to_token_ids(sentence, vocab)
        # Pad out (or truncate) the input gloss ids.
        padded_ids = np.asarray(pad_sequence(token_ids, max_seq_len))
        ...
        with tf.device("/cpu:0"):
            with tf.Session() as sess:
                (input_node, target_node, predictions, loss, vocab,
                 rev_vocab) = restore_model(sess, FLAGS.save_dir, dataset,
                                            out_form="cosine")

                if FLAGS.evaluate:
                    evaluate_model(sess, FLAGS.data_dir,
                                   input_node, target_node,
                                   predictions, loss, rev_vocab, embs=pre_embs, out_form="cosine")

                # Load the final saved model and run querying routine.
                query_model(sess, input_node, predictions,
                            vocab, rev_vocab, dataset, FLAGS.max_seq_len, embs=pre_embs,
                            out_form="cosine")
